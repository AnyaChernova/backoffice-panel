<template>
	...
	>
		<template #headerLeft>
			<div class="d-text-title-xs">Файлы проекта</div>
		</template>
...
export default {
	...
	data() {
		return {
			entityModalForm: 'ModalFormProjectFile',
			entityModalFormParams: {
				project: this.project,
			},
			entityResource: this.$api.projectFile,
			filters: {
	...
	methods: {
		onClickDelete(item) {
			this.runAction({
				...
				confirmSubTitle: 'Вы уверены, что хотите удалить файл проекта?',
